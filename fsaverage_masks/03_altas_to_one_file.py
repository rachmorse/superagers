...
"""
Python script to combine atlas components (left/right subcortical regions and Schaefer cortical regions)
into a single atlas in T1 space.
"""
...
def get_subjects_to_process(output_folder):
    ...
    for subject in os.listdir(output_folder):
        ...
        if t1_file_path.exists() and subcort_left_file_path.exists() and subcort_right_file_path.exists() and not output_file_path.exists():
...
def process_subject(output_folder, subject, ses):
    """Process a single subject to create a combined Glasser374 atlas.
    ...
    """
    try:
        subject_dir = os.path.join(output_folder, subject, ses)
                
        logger.info(f"Processing subject {subject}")
        
        # Load the individual brain atlas components
        cortical = nib.load(Path(f"{output_folder}/{subject}_schaefer_volumetric_t1.nii.gz")).get_fdata()
        left_subcortical = nib.load(Path(f"{output_folder}/{subject}_left_subcortical14_t1.nii.gz")).get_fdata()
        right_subcortical = nib.load(Path(f"{output_folder}/{subject}_right_subcortical14_t1.nii.gz")).get_fdata()
        
        # Adjust values to create non-overlapping ranges for each region
        cortical_adj = np.where(cortical != 0, cortical, 0)                                   # cortical: values from 1 to 200 (keeping 0 as 0)
        ...
        affine = nib.load(cortical).affine
        
        # Create a matrix to store the final result
        result = np.zeros_like(cortical)
        
        # Create a stacked matrix for comparison
        stacked = np.stack([cortical_adj, left_subcortical_adj, right_subcortical_adj], axis=-1)
        ...
        with open(output_file, "a") as file:
            ...
            for idx, voxel_values in enumerate(overlap_values[0], start=1):
                ...
                if cortical_value != 0: cortical_perc = n_values_overlaped*100/np.sum(cortical_adj==cortical_value)
                ...
                new_line = f"{subject}, {idx}, {n_values_overlaped}, {cortical_value}, {left_subcortical_value}, {right_subcortical_value}, {cortical_perc}, {left_subcortical_perc}, {right_subcortical_perc}\n"
                file.write(new_line)

        # For voxels with overlap, keep the value from the area with the highest percentage of affected voxels
        for idx, voxel_values in enumerate(overlap_values[0], start=1):
            cortical_value = voxel_values[0]
            left_subcortical_value = voxel_values[2]
            right_subcortical_value = voxel_values[3]
            n_values_overlaped = overlap_values[1][idx-1]
            cortical_perc = 0.
            ...
            max_perc_idx = np.argmax(np.array([cortical_perc, left_subcortical_perc, right_subcortical_perc]))
            
            # Assign values based on the region with highest percentage
            result[(overlap_mask) &
                      (stacked[:,:,:,0]==cortical_value) &
                      (stacked[:,:,:,2]==left_subcortical_value) &
                      (stacked[:,:,:,3]==right_subcortical_value)] = np.array([cortical_value, left_subcortical_value, right_subcortical_value])[max_perc_idx]
        ...
        if unique_values.shape[0] != 215:  # 215 regions + background (0)
            logger.warning(f"Subject {subject} has incomplete atlas: {unique_values.shape[0]-1} regions instead of 374")
        ...
        nib.save(nib.Nifti1Image(result, affine), Path(f"{output_folder}/{subject}_schaefer_subcortical_t1.nii.gz"))
...
def main():
    ...
    logger.info(f"Script executed by: {os.environ.get('USER', 'rachmorse')}")
