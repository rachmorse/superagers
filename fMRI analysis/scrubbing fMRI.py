...
def main(multi=False):
    ...
    Notes:
        - The default session timepoint is set to "02".
    ...
    """
    # Define the session timepoint and directories for data input. Change these as needed for the data being processed
    ses = "01"
    ...
    # First, concatenate all framewise_displ.txt files (per subject) into a single DataFrame so that there is one row per subject
    ...
    # Second, visualize what different thresholds would look like in the data 
    analyze_threshold(all_fwd_df, 0.2)
    analyze_threshold(all_fwd_df, 0.5)

    # Third, work on scrubbing the BOLD images
    ...
    if multi:
        with Pool(4) as pool:
            pool.starmap(process_subject, [(subject, ses, root, output_data, error_log) for subject in todo])
    else:
        for subject in todo:
            process_subject(subject, ses, root, output_data, error_log)
